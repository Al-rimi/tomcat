...
> 版本 4.1.0 - 新增多项目工作区感知、应用创建/部署/运行/删除、实例与应用一体化管理、模板+前端脚手架、部署状态动画、双语本地化支持等。
...
### 新增
- 多项目工作区检测与智能切换，支持多根工作区的项目管理。
- 应用树视图新增 `+` 脚手架 UI，支持模板+前端类型组合（JSP/Thymeleaf/React/Vue/Angular/无）。
- 实例级部署动画（`sync~spin`）和全局刷新按钮。
- 应用树工具栏新增创建/刷新操作，一键生成项目。
- 新建应用流程和所有新增功能均支持中英文本地化。
- 生成项目自动包含 MIT 协议、README、并检测 Java/Tomcat/Node/NPM/平台版本。
- 修复前端模板 `public/index.html` 路径创建（解决 ENOENT）。

### 变更
- 扩展停用时不再强制关闭非托管 Tomcat 进程，托管实例支持跟踪与持久化。
- 部署逻辑优化：优先复用已运行实例（同应用复用、空闲实例、新建实例）。
- 浏览器处理增强：可用性检测、首选浏览器回退、更安全的启动/超时。
- 大规模本地化：所有用户界面文本迁移至运行时 NLS，支持中英文。

### 修复
- 修复本地化和 JSON/TypeScript 相关问题。
- 优化端口分配与更新，避免不必要的重启并增强端口占用检测。
